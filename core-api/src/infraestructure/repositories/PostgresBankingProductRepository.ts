...
export class PostgresBankingProductRepository
  implements IBankingProductRepository
{
  constructor(private readonly prisma: PrismaClient) {}

  async findAll(): Promise<BankingProduct[]> {
    const bankingProducts = await this.prisma.bankingProduct.findMany();
    return bankingProducts.map(
      bankingProduct =>
        new BankingProduct(bankingProduct.id, bankingProduct.bankingProductName)
    );
  }
