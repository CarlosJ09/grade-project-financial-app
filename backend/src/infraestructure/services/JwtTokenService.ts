...
export class JwtTokenService implements ITokenService {
    ...
    constructor() {
        ...
        if (!process.env.JWT_ACCESS_SECRET || !process.env.JWT_REFRESH_SECRET) {
            console.warn('⚠️ JWT secrets not found in environment variables. Using default values for development.');
        }
    }
    ...
    verifyAccessToken(token: string): { userId: string } | null {
        try {
            const decoded = jwt.verify(token, this.accessTokenSecret) as { userId: string };
            return decoded;
        } catch (error) {
            return null;
        }
