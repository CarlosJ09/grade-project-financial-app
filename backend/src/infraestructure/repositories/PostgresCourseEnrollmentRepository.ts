...
export class PostgresCourseEnrollmentRepository implements ICourseEnrollmentRepository {
    constructor(private readonly prisma: PrismaClient) { }

    async findAll(): Promise<CourseEnrollment[]> {
        const courseEnrollments = await this.prisma.courseEnrollment.findMany();
        return courseEnrollments.map(courseEnrollment => new CourseEnrollment(
            courseEnrollment.id,
    ...
    async create(entity: Omit<CourseEnrollment, 'id'>): Promise<CourseEnrollment> {
    ...
    async update(id: string, entity: Partial<Omit<CourseEnrollment, 'id'>>): Promise<CourseEnrollment> {
