...
export class PostgresUserBankRepository implements IUserBankRepository {
    ...
    async findAll(): Promise<UserBank[]> {
        ...
        return userBanks.map(userBank => new UserBank(
            userBank.id,
            userBank.userId,
            userBank.bankId,
            userBank.alias,
    ...
    async create(entity: Omit<UserBank, 'id' | 'createdAt' | 'updatedAt'>): Promise<UserBank> {
    ...
    async update(id: string, entity: Partial<Omit<UserBank, 'id' | 'createdAt' | 'updatedAt'>>): Promise<UserBank> {
